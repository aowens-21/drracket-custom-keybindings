...
(test-kb-program
 ...
 (λ (result-val result-pos result-text)
   (check-equal? result-text "(board (_ b _ b _ b _ b)\n       (b _ b _ b _ b _)\n       (_ b _ b _ b _ b)\n       (_ _ _ _ _ _ _ _)\n       (_ _ _ _ _ r _ _)\n       (r _ r _ r _ _ _)\n       (_ r _ r _ r _ r)\n       (r _ r _ r _ r _))")) 
 #:setup-proc (λ (editor)
                (send editor insert initial-editor-content)
                ;; needed to guarantee good positions for the checker keybindings
                (send editor tabify-all)
                (send editor set-position 141)))
...
(test-kb-program
 ...
 #:setup-proc (λ (editor)
                ...
                (send editor tabify-all)
                (send editor set-position 141)
                (send editor move-position 'up 'simple)))
